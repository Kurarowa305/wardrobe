...
check(
  "TFS-06",
  "編集画面が現在テンプレートに含まれる削除済み服も選択肢として保持する",
  includes(editTarget, 'const currentDeletedItems = (templateQuery.data?.clothingItems ?? [])') &&
    includes(editTarget, '.filter((item) => item.deleted && !activeItemIds.has(item.clothingId))') &&
    includes(editTarget, 'return [...activeItems, ...currentDeletedItems];'),
  "編集画面で削除済み服を保持するための選択肢マージ処理が不足しています",
);

check(
  "TFS-07",
  "共通フォームがテンプレ名入力、服複数選択、削除済み表示、空状態、送信エラー表示を持つ",
  includes(formTarget, 'placeholder={strings.placeholders.name}') &&
    ...
    includes(formTarget, 'strings.messages.deletedClothing') &&
    includes(formTarget, 'strings.messages.deletedClothingDescription') &&
  ...
  "共通フォームの入力UIまたは削除済み表示/状態表示が不足しています",
);
...
check(
  ...
  includes(stringsTarget, 'nameRequired: "テンプレート名を入力してください。"') &&
    ...
    includes(stringsTarget, 'deletedClothing: "削除済み"') &&
    includes(stringsTarget, 'deletedClothingDescription: "削除済みの服も現在の構成として保持しています。不要な場合はチェックを外してください。"') &&

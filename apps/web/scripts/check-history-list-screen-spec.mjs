...
check(
  "HLS-05",
  "履歴カードが共通サムネイルレール経由で no image/削除済み表示に対応する",
  includes(
    cardTarget,
    'import { ThumbnailRail } from "@/components/app/shared/ThumbnailRail";',
  ) &&
    includes(cardTarget, "<ThumbnailRail") &&
    includes(
      cardTarget,
  ...
  "履歴カードの共通サムネイルレール利用または no image 表示実装が不足しています",
);
...
check(
  ...
  includes(cardTarget, "const HISTORY_THUMBNAIL_LIMIT = 4;") &&
    includes(cardTarget, "limit={HISTORY_THUMBNAIL_LIMIT}") &&
    includes(
      "src/components/app/shared/ThumbnailRail.tsx",

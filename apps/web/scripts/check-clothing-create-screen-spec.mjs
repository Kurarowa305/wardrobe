...
check("CCS-06", "保存成功時に服一覧へ遷移し、genre/tagIds を payload に含める", includes(target, 'await createMutation.mutateAsync({ name: trimmedName, genre: genre as ClothingGenreDto, imageKey: uploadedImageKey, tagIds: selectedTagIds });') && includes(target, 'router.push(appendOperationToast(ROUTES.clothings(wardrobeId), OPERATION_TOAST_IDS.clothingCreated));'), "保存 payload または遷移が不足しています");

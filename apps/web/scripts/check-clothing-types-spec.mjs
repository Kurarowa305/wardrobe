...
check("CT-03", "一覧/詳細レスポンスDTOに genre / tagIds と nextCursor を含む", includes("src/api/schemas/clothing.ts", 'export type ClothingListItemDto = Pick<ClothingDto, "clothingId" | "name" | "genre" | "imageKey" | "tagIds">;') && includes("src/api/schemas/clothing.ts", 'tagIds: ItemTagIdDto[];') && includes("src/api/schemas/clothing.ts", 'nextCursor: string | null;'), "一覧DTOの genre / tagIds または nextCursor が不足しています");
check("CT-04", "画面表示用VMが genre / tagIds を保持する", exists("src/features/clothing/types.ts") && includes("src/features/clothing/types.ts", 'genre: ClothingGenreDto;') && includes("src/features/clothing/types.ts", 'genre: dto.genre,') && includes("src/features/clothing/types.ts", 'tagIds: ItemTagIdDto[];') && includes("src/features/clothing/types.ts", 'tagIds: dto.tagIds,'), "VMの genre / tagIds 定義が不足しています");

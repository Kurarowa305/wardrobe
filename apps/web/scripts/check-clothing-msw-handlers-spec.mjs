...
check("CM-03", "一覧 handler が genre/limit を受け取り cursor を使わず items を返す", includes("src/mocks/handlers/clothing.ts", 'const genreParam = url.searchParams.get("genre");') && includes("src/mocks/handlers/clothing.ts", 'const limit = parseLimit(url.searchParams.get("limit"));') && !includes("src/mocks/handlers/clothing.ts", 'parseCursor(') && !includes("src/mocks/handlers/clothing.ts", 'nextCursor') && includes("src/mocks/handlers/clothing.ts", 'return HttpResponse.json<ClothingListResponseDto>({ items: items.slice(0, limit) });'), "genre/limit 実装または cursor 削除が不足しています");
...
check("CM-07", "create/update handler が genre を保存し、登録上限100件を超えない", includes("src/mocks/handlers/clothing.ts", 'genre: payload.genre,') && includes("src/mocks/handlers/clothing.ts", 'if (payload.genre !== undefined) target.genre = payload.genre;') && includes("src/mocks/handlers/clothing.ts", 'const MAX_CLOTHING_COUNT = 100;') && includes("src/mocks/handlers/clothing.ts", 'CLOTHING_LIMIT_EXCEEDED'), "genre 保存処理または登録上限100件制御が不足しています");

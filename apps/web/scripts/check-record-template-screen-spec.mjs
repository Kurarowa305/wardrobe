...
function check(id, description, predicate, failureDetail) {
  try {
    ...
    results.push({
      ...
      description,
      passed,
      detail: passed ? "ok" : failureDetail,
    });
  } catch (error) {
...
check(
  ...
  () =>
    ...
    includes(target, "selectedTemplateId"),
  "テンプレート一覧取得または単一選択 UI の実装が不足しています",
);
check(
  "RTS-05",
  "テンプレート一覧が共通サムネイルレール付き選択リストとして描画される",
  () =>
    includes(
      target,
    ...
    includes(target, "grid-cols-[minmax(0,1fr)_40px]") &&
    includes(target, 'type="radio"'),
  "テンプレート一覧の共通サムネイル付き選択リスト実装が不足しています",
);
check(
  "RTS-06",
...
check(
  ...
  "記録成功後にホームへ戻る",
  () =>
    includes(target, "router.push(") &&
    includes(target, "appendOperationToast(") &&
    includes(target, "ROUTES.home(wardrobeId)") &&
    includes(target, "OPERATION_TOAST_IDS.historyCreated"),
...
check(
  ...
  () =>
    !includes(stringsTarget, 'selected: "選択中のテンプレート"') &&
    includes(stringsTarget, 'dateRequired: "日付を入力してください。"') &&
    includes(
      stringsTarget,
      'templateRequired: "テンプレートを選択してください。"',
    ) &&
    includes(stringsTarget, 'loading: "テンプレートを読み込んでいます…"') &&
    includes(
      stringsTarget,

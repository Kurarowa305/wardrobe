...
check(
  ...
  includes(target, 'import { formatHistoryDate } from "@/features/history/date";') &&
    includes(target, '{formatHistoryDate(item.date)}') &&
...
check(
  ...
  includes(target, 'className="flex items-start justify-between gap-3"') &&
...
check(
  ...
  includes(target, 'const combinationTitle = item.clothingItems.map((clothingItem) => clothingItem.name).join("+");') &&
    includes(target, 'item.inputType === "template"') &&
    includes(target, 'item.name ?? HISTORY_STRINGS.list.messages.combinationSummary') &&
...
check(
  ...
  includes(target, 'const HISTORY_CARD_TITLE_MAX_LENGTH = 15;') &&
    includes(target, 'title.length > HISTORY_CARD_TITLE_MAX_LENGTH') &&
    includes(target, 'title.slice(0, HISTORY_CARD_TITLE_MAX_LENGTH)}...'),

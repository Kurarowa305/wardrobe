...
check("RCS-04", "服一覧取得にジャンル別 clothing query hook を用い、複数選択 checkbox を描画する", includes(target, 'const topsQuery = useClothingList(wardrobeId, { genre: "tops"') && includes(target, 'const bottomsQuery = useClothingList(wardrobeId, { genre: "bottoms"') && includes(target, 'const othersQuery = useClothingList(wardrobeId, { genre: "others"') && includes("src/components/app/screens/ClothingListCard.tsx", 'type="checkbox"'), "ジャンル別取得または複数選択 UI が不足しています");
check("RCS-05", "服一覧がジャンル別サムネイル付き選択セクションとして描画される", includes(target, 'ClothingGenreSection') && includes("src/components/app/screens/ClothingListCard.tsx", 'grid-cols-[56px_minmax(0,1fr)_40px]') && includes("src/components/app/screens/ClothingGenreSection.tsx", 'ClothingGenreIcon genre={genre}'), "ジャンル別サムネイル付きセクションが不足しています");
...
check("RCS-07", "ジャンルごとにスクロール下端で自動読み込みする", includes(target, 'onLoadMore={state.nextCursor !== null && !query.isFetching ? () => handleLoadMore(genre) : undefined}') && includes("src/components/app/screens/ClothingGenreSection.tsx", '<AutoLoadTrigger'), "ジャンル別自動読み込みが不足しています");

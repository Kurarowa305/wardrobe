...
check("LSS-01", "履歴一覧画面が自動読み込み trigger を表示する", includes(historyTarget, '<AutoLoadTrigger') && includes(historyTarget, 'loadingLabel={HISTORY_STRINGS.list.messages.loading}'), "履歴一覧の自動読み込み trigger が不足しています");
...
check("LSS-03", "テンプレート一覧画面が自動読み込み trigger を表示する", includes(templateTarget, '<AutoLoadTrigger') && includes(templateTarget, 'loadingLabel={TEMPLATE_STRINGS.list.messages.loading}'), "テンプレート一覧の自動読み込み trigger が不足しています");
...
check("LSS-05", "服一覧画面はジャンルセクション単位で自動読み込み trigger を表示する", includes(clothingTarget, '<div className="grid gap-4">') && includes("src/components/app/screens/ClothingGenreSection.tsx", '<AutoLoadTrigger'), "服一覧のジャンル別自動読み込みレイアウトが不足しています");

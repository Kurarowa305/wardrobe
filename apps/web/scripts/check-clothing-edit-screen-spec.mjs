...
check("CES-06", "更新成功時に詳細画面へ遷移し、genre/tagIds を payload に含める", includes(target, 'await updateMutation.mutateAsync({ name: trimmedName, genre: genre as ClothingGenreDto, imageKey: nextImageKey, tagIds: selectedTagIds });') && includes(target, 'router.push(appendOperationToast(ROUTES.clothingDetail(wardrobeId, clothingId), OPERATION_TOAST_IDS.clothingUpdated));'), "更新 payload または遷移が不足しています");

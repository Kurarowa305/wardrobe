const WEEKDAYS_JA = ["日", "月", "火", "水", "木", "金", "土"] as const;

export function formatHistoryDate(date: string) {
  if (!/^\d{8}$/.test(date)) {
    return date;
  }

  const year = Number(date.slice(0, 4));
  const monthIndex = Number(date.slice(4, 6)) - 1;
  const day = Number(date.slice(6, 8));
  const weekday = WEEKDAYS_JA[new Date(year, monthIndex, day).getDay()];

  return `${date.slice(0, 4)}/${date.slice(4, 6)}/${date.slice(6, 8)} (${weekday})`;
}

export function formatLastWornDate(lastWornAt: number | null, neverWornLabel: string) {
  if (lastWornAt === null) {
    return neverWornLabel;
  }

  const date = new Date(lastWornAt);
  const weekday = WEEKDAYS_JA[date.getDay()];
  const formatted = new Intl.DateTimeFormat("ja-JP", {
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
  }).format(date);

  return `${formatted} (${weekday})`;
}

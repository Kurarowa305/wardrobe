...
const HOME_HISTORY_THUMBNAIL_LIMIT = 4;
...
function HomeHistoryCard({ wardrobeId, item }: { wardrobeId: string; item: HistoryListItem }) {
  const visibleThumbnails = item.clothingItems.slice(0, HOME_HISTORY_THUMBNAIL_LIMIT);
  const hiddenCount = Math.max(item.clothingItems.length - HOME_HISTORY_THUMBNAIL_LIMIT, 0);

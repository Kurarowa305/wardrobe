...
function TemplatePreviewCard({
  ...
  return (
    <label
      className={[
        "grid gap-3 rounded-md border px-3 py-3 text-sm text-slate-900",
        selected ? "border-slate-400 bg-slate-50" : "border-slate-200 bg-white",
        onSelect ? "cursor-pointer" : "",
      ].join(" ")}
    >
      {onSelect ? (
        <input type="radio" name="templateId" checked={selected} onChange={onSelect} className="sr-only" />
      ) : null}
...
export function RecordByTemplateScreen({ wardrobeId }: RecordByTemplateScreenProps) {
  ...
  const selectableTemplateItems = useMemo(
    () => templateItems.filter((item) => item.templateId !== selectedTemplateId),
          ...
          <fieldset className="grid gap-3 rounded-md border border-slate-200 bg-white p-3">
            ...
            {selectedTemplate ? (
              <div className="grid gap-2 rounded-md border border-slate-200 bg-slate-50 p-3">
                ...
                <TemplatePreviewCard item={selectedTemplate} selected />
              </div>
            ) : null}

            {selectableTemplateItems.length > 0 ? (
              <div className="grid gap-2">
                {selectableTemplateItems.map((item) => (
                  <TemplatePreviewCard
                    key={item.templateId}
                    item={item}

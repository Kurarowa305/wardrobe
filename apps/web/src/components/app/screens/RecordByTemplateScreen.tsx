...
export function RecordByTemplateScreen({ wardrobeId }: RecordByTemplateScreenProps) {
  ...
  const [date, setDate] = useState(formatTodayForInput);
  ...
  const showTemplateEmpty = !showTemplateLoading && !showTemplateError && !hasTemplateItems;
  const showDateError = touched && date.length === 0;
  const showTemplateRequiredError = touched && selectedTemplateId.length === 0;
  const canSubmit = date.length > 0 && selectedTemplateId.length > 0 && !createHistoryMutation.isPending;
    ...
    <AppLayout title={RECORD_STRINGS.byTemplate.title} backHref={ROUTES.recordMethod(wardrobeId)}>
      <ScreenCard>
        <form className="grid gap-3" onSubmit={handleSubmit} noValidate>
            ...
            {hasTemplateItems ? (
              <div className="grid gap-2">
                {templateItems.map((item) => {
                  const checked = item.templateId === selectedTemplateId;
          ...
          <Button type="submit" className="w-full text-sm font-medium" disabled={createHistoryMutation.isPending}>

...
import { TemplateForm, type TemplateSelectableClothingItem } from "./TemplateForm";
...
export function TemplateEditScreen({ wardrobeId, templateId }: TemplateEditScreenProps) {
  ...
  const clothingItems = useMemo<TemplateSelectableClothingItem[]>(() => {
    const activeItems = (clothingListQuery.data?.items ?? []).map((item) => ({
      clothingId: item.clothingId,
      name: item.name,
      deleted: false,
    }));
    const activeItemIds = new Set(activeItems.map((item) => item.clothingId));
    const currentDeletedItems = (templateQuery.data?.clothingItems ?? [])
      .filter((item) => item.deleted && !activeItemIds.has(item.clothingId))
      .map((item) => ({
        clothingId: item.clothingId,
        name: item.name,
        deleted: true,
    ...
    return [...activeItems, ...currentDeletedItems];
  }, [clothingListQuery.data?.items, templateQuery.data?.clothingItems]);

...
服追加画面・服編集画面において、画像ファイル選択時のプレビュー表示とクリア導線、ならびに `imageKey` 入力廃止後の値連携仕様（追加時はアップロード結果の `imageKey` を利用）を静的検査で担保する。
...
5. 服追加時にアップロード結果の `imageKey` を保存payloadへ設定すること
...
### CIS-05: 服追加時はアップロード結果の imageKey を保存payloadに設定する

...
### MF-06 MSW 起動関数が NEXT_PUBLIC_ENABLE_MSW=true のときだけ一度だけ起動する
- 観点: 本番環境への影響回避と、ローカル/検証環境のみでの明示有効化、多重起動防止
- 期待結果:
  - `src/mocks/start.ts` に `shouldEnableMockServiceWorker` があり、`NEXT_PUBLIC_ENABLE_MSW === "true"` を条件にする

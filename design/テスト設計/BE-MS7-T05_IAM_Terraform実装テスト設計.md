...
- `infra/terraform/app/iam.tf` と `infra/terraform/app/lambda.tf` が BE-MS7-T05 の完了条件（Lambda execution role、presign のみ S3 権限、他 domain は DynamoDB 中心、CloudWatch Logs 権限あり）を満たすことを継続検証する。
...
| TC-04 | 権限分離（presign vs その他） | `iam.tf` を検査 | `wardrobe` / `clothing` / `template` / `history` は DynamoDB 権限、`presign` は S3 権限で分離される |

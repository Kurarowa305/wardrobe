...
* `ワードローブ作成` を除く全画面は同一 `wardrobeId` 文脈（`/wardrobes/{wardrobeId}/...`）で遷移する
* `CreateWardrobe --> Home` は URLとしては `/wardrobes/new` から `/wardrobes/{wardrobeId}/home` への遷移を意味する
* 履歴詳細は `?from=home|histories` で戻り先文脈を保持する
* `from` が未指定または不正値の場合の戻り先は `HistoryList`（履歴一覧）とする
...
' NOTE:
' - CreateWardrobe以外は /wardrobes/{wardrobeId}/... 配下
' - HistoryDetail は ?from=home|histories で戻り先文脈を保持

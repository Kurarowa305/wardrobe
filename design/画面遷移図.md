...
* `ワードローブ作成` を除く全画面は同一 `wardrobeId` 文脈（`?wardrobeId=...`）で遷移する
* `CreateWardrobe --> Home` は URLとしては `/wardrobes/new` から `/home?wardrobeId={wardrobeId}` への遷移を意味する
...
' NOTE:
' - CreateWardrobe以外は wardrobeId をクエリ (?wardrobeId=...) で受け渡す
